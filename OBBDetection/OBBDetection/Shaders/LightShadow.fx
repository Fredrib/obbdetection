...
//
// Modified base code - Jonathan Tompson, Summer 2009
//
//=============================================================================
...
// static const float SMAP_SIZE = 510.0f;
...
technique BuildShadowMapTech
{
	pass P0
	{
        ...
        pixelShader  = compile ps_2_a BuildShadowMapPS();
	}
}
...
uniform extern float	 gShadowMapSize;
                     ...
                     float4 projTex : TEXCOORD4) : COLOR
{
	...
	float t  = pow(max(dot(r, toEyeW), 0.000001f), gMtrl.specPower);
	...
	float spot = pow(max( dot(-lightVecW, gLight.dirW), 0.000001f), gLight.spotPower);
    ...
    float2 texelpos = gShadowMapSize * projTex.xy;
	...
	// SHADOW COEFFICIENTS ARE INTERPOLATED!!! --> REMOVE ALIASING
...
technique LightShadowTech
{
    pass P0
    {
        ...
        pixelShader  = compile ps_2_a LightShadowPS();
    }
}
