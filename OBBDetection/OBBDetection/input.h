...
public:
    ...
    keyboard(); // New Jonathan
    ~keyboard(); // New Jonathan

	void InitKeyboard(void); // New Jonathan
...
public:
	...
	void InitMouse(void); // New Jonathan
...
private:
	...
	// std::vector< std::pair<float,float> > m_historyBuffer;  // GAVE VERY WIERD RUNTIME ERRORS IN VC2008 EXPRESS
    // float m_historyBuffer[2][10];
	float m_historyBuffer[20];

    // std::pair<float,float> m_mouseMovement[2]; // GAVE VERY WIERD RUNTIME ERRORS IN VC2008 EXPRESS
	// float m_mouseMovement[2][2]; 
	float m_mouseMovement[4]; 
};
